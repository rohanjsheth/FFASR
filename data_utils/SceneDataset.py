...
class SceneDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> RenderedScene:
        ...
        if rng.random() < self._clean_probability:
            scene = render_clean_scene(idx, self._speech_ds, self._sample_rate)
            if self._teacher:
                scene["teacher_audio"] = scene["audio"]
            return scene

        recipe = sample_scene_recipe(idx, self._noise_ds, self._groups, rng, self._number_of_noises)
        return render_scene_from_recipe(
            recipe, self._speech_ds, self._noise_ds, self._rir_ds, self._sample_rate,
